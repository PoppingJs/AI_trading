...
<body>
  ...
  <main class="workspace">
    <aside class="panel left">
      ...
      <section class="analysis" id="analysisSection"><h3>分析总结</h3><div class="analysis-lines" id="failureSummary"></div></section>
      <section class="curve" id="curveSection"><div class="curve-head"><h3>总收益曲线</h3><span id="curvePeriod">开始时间 - 结束时间</span></div><div id="equityChart"></div></section>
    </aside>
    <section class="right">
      ...
      <article class="panel"><div class="section-head"><h2>成交记录</h2><small id="tradesAsOf">尚未回放</small></div><div class="table-wrap"><table><thead><tr><th>币种</th><th>方向</th><th>杠杆</th><th>开仓均价</th><th>平仓均价</th><th>数量</th><th>止损</th><th>止盈</th><th>收益率</th><th>实现盈亏</th><th>手续费</th><th>开仓时间</th><th>平仓时间</th><th>入场位置</th><th>出场原因</th></tr></thead><tbody id="trades"><tr class="empty"><td colspan="15">暂无数据</td></tr></tbody></table></div></article>
    </section>
  </main>
  <script>
    ...
    function resetRunningView(){
      ...
      summary.innerHTML='';
      document.getElementById('curvePeriod').textContent=`${document.getElementById('startDate').value} - ${document.getElementById('endDate').value}`;
      document.getElementById('equityChart').innerHTML='';
      renderPositions([],'暂无数据');
      renderTrades([],'暂无数据');
    ...
    function updateJob(job){
      document.getElementById('runButton').textContent=`回测中 ${job.progress||0}%`;
      ...
      renderTrades(account.fills||[],'截至当前模拟时间暂无成交记录');
    }

    function showError(message){
      const host=document.getElementById('failureSummary');
      host.innerHTML=`<div class="analysis-line">${esc(message||'未知错误')}</div>`;
    ...
    function renderResult(){
      ...
      renderTrades(account.fills||[],'本区间没有成交记录');
    }
    ...
    function exitReasonText(value){
      ...
      const details=[
        ...
        ['4h body closed below support or ema/boll zone','4小时实体跌破支撑或EMA/BOLL区域'],
        ['4h body closed above resistance or ema/boll zone','4小时实体突破压力或EMA/BOLL区域'],
    ...
    function renderTrades(fills,emptyText='暂无成交记录'){
      const rows=[...(fills||[])];
